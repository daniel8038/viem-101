平常那 我们在做 数据查询的时候 一般都是读函数，uniswapV3 比较有意思的合约就是 quote 合约。他把数据的查询写成了一个写函数，那我们知道一个写函数的成功调用其实会对区块链状态有影响的。那 uniswap 里边就使用了 revert 来保证这个询价函数绝对不能执行成功。所以 模拟调用 或者 直接调用 都可以拿到 revert 结果 然后拿到返回的数据。主要原因还是因为 uniswapV3 需要复杂的计算，读函数不能满足，所以通过写函数和 revert 实现数据的完整和正确性
